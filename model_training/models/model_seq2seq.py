...
# =============================================================================
    ac_input = Input(
        ...
        name='ac_input')
    rl_input = Input(
        shape=(vec['encoder_input_data']['roles'].shape[1], ),
        name='rl_input')
    t_input = Input(
        shape=(vec['encoder_input_data']['times'].shape[1], 1),
        name='t_input')
...
# =============================================================================
    ...
    l1_c1, state_l1_c1, state_l1_c1 = LSTM(
        args['l_size'],
    ...
    l1_c3, state_l1_c3, state_l1_c3 = LSTM(
        args['l_size'],
...
# =============================================================================
    dec_ac_input = Input(
        ...
        name='dec_ac_input')
    dec_rl_input = Input(
        shape=(vec['decoder_input_data']['roles'].shape[1], ),
        name='dec_rl_input')
    dec_t_input = Input(
        shape=(vec['decoder_input_data']['times'].shape[1], 1),
        name='dec_t_input')

# =============================================================================
#    Embedding layer for categorical attributes        
# =============================================================================
    dec_ac_embedding = Embedding(ac_weights.shape[0],
                            ac_weights.shape[1],
                            weights=[ac_weights],
                            input_length=vec['decoder_input_data']['activities'].shape[1],
                            trainable=False, name='dec_ac_embedding')(dec_ac_input)

    dec_rl_embedding = Embedding(rl_weights.shape[0],
                            rl_weights.shape[1],
                            weights=[rl_weights],
                            input_length=vec['decoder_input_data']['roles'].shape[1],
                            trainable=False, name='dec_rl_embedding')(dec_rl_input)

    dec_merged = Concatenate(name = 'dec_concatenated', axis=2)(
        [dec_ac_embedding, dec_rl_embedding])

    l2_c1, _, _ = LSTM(
        args['l_size'],
        return_sequences=True,
        return_state=True,
        implementation=args['imp'])(dec_merged, initial_state=encoder_states_l1_c1)
    ...
    l2_c3, _, _ = LSTM(
        ...
        return_state=True,
        dropout=0.2,
        implementation=args['imp'])(dec_t_input, initial_state=encoder_states_l1_c3)

# =============================================================================
#    Output
# =============================================================================
    dec_act_output = Dense(len(args['index_ac']), activation='softmax',
                       name='act_output')(l2_c1)
    dec_rl_output = Dense(len(args['index_rl']), activation='softmax',
                       name='rl_output')(l2_c1)
    if ('dense_act' in args) and (args['dense_act'] is not None):
        dec_time_output = Dense(1, activation=args['dense_act'],
                            kernel_initializer='glorot_uniform',
    ...
    model = Model(inputs=[ac_input, rl_input, t_input,
                          dec_ac_input, dec_rl_input, dec_t_input],
