...
#    # Input vectorization
    x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp, max_tbtw = vectorization(log_df_train, ac_index, rl_index, args)
    
#    [print(x_ac_inp[i], xt_inp[i], yt_inp[i], sep=' ') for i in range(0, 10)]
    # Parameters export
    ...
    if args['model_type'] == 'joint':
        mj.training_model(x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp, ac_weights, rl_weights, output_folder, args)
    elif args['model_type'] == 'shared':
        msh.training_model(x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp, ac_weights, rl_weights, output_folder, args)
    elif args['model_type'] == 'specialized':
        msp.training_model(x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp, ac_weights, rl_weights, output_folder, args)
    elif args['model_type'] == 'concatenated':
        mcat.training_model(x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp, ac_weights, rl_weights, output_folder, args)
    elif args['model_type'] == 'shared_cat':
        mshcat.training_model(x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp, ac_weights, rl_weights, output_folder, args)
...
#    # TODO Considerar usar otra estructura que no sean listas independientes para reducir el numero de variables y ademas esto se puede volver inmanejable
#    # TODO Configurar dos vectores uno para entradas continuas y otro para discretas, estas podrias crearse de forma automatica o haciendo explicito el tipo de dato
    x_ac_inp, x_rl_inp, xt_inp, y_ac_inp, y_rl_inp, yt_inp = list(), list(), list(), list(), list(), list()
