...
def main(argv):
    ...
    if not argv:
        # Event-log filename
        settings['gl']['file'] = 'Production.xes'
    ...
    settings['tm']['arr_support'] = [0.01, 0.05]  # [0..1]
...
def define_general_settings(settings):
    ...
    settings['gl']['read_options'] = {'timeformat': '%Y-%m-%d %H:%M:%S',
                                      'column_names': column_names,
                                      'one_timestamp': False,
                                      'filter_d_attrib': True}
    # Folders structure
    settings['gl']['input'] = 'inputs'
    settings['gl']['output'] = os.path.join('outputs_files', 'Simod', sup.folder_id())
