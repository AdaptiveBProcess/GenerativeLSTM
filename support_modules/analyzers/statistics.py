...
def role_statistics(process_stats):
    ...
    for key, group in itertools.groupby(data, key=lambda x:x['source']):
        ...
        for key2, group2 in itertools.groupby(data2, key=lambda x:x['run_num']):
            ...
            for key3, group3 in itertools.groupby(data3, key=lambda x:x['caseid']):
                # Group by task
                data4 = sorted(list(group3), key=lambda x:x['resource'])
                for key4, group4 in itertools.groupby(data4, key=lambda x:x['resource']):
                    values = list(group4)
                    group_processing_time = [x['processing_time'] for x in values]
                    case_list.append(dict(source=key , run_num=key2, caseid=key3, resource=key4,
    ...
    for key, group in itertools.groupby(data, key=lambda x:x['source']):
        ...
        for key2, group2 in itertools.groupby(data2, key=lambda x:x['run_num']):
            # Group by task
            data4 = sorted(list(group2), key=lambda x:x['resource'])
            for key4, group4 in itertools.groupby(data4, key=lambda x:x['resource']):
                values = list(group4)
                group_processing_time = [x['processing_time'] for x in values]
                run_list.append(dict(source=key , run_num=key2, resource=key4,
    ...
    for key, group in itertools.groupby(data, key=lambda x:x['source']):
        values = list(group)
        data2 = sorted(values, key=lambda x:x['resource'])
        # Group by task
        for key2, group2 in itertools.groupby(data2, key=lambda x:x['resource']):
            values2 = list(group2)
            group_processing_time = [x['processing_time'] for x in values2]
            role_list.append(dict(source=key ,resource=key2, processing_time=group_processing_time))
    return role_list
