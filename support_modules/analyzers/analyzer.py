# -*- coding: utf-8 -*-
from support_modules.analyzers.analyzer_ui import analyzer_interface as ui
import numpy as np
import scipy.stats as st
import support as sup
import os

from support_modules.analyzers import statistics as sta
...
def mean_confidence_interval(data, confidence=0.99):
    ...
    h = se * st.t._ppf((1+confidence)/2.0, n-1)
    return h
...
def calculate_role_durations(sources, roles, role_time_use):
    ...
    for role in roles:
        for source in sources:
            values = list(filter(lambda x: x['source'] == source and x['resource'] == role, role_time_use))
            if len(values) > 0:
...
def calculate_process_duration(sources, durations):
    ...
    for source in sources:
        ...
        if len(values) > 0:
            process_durations.append(dict( raw_processing=values[0]['processing_time'], processing_time = np.median(values[0]['processing_time']),
            waiting_time = np.median(values[0]['waiting_time']), multitasking = np.median(values[0]['multitasking']), source=source,
            pmci=mean_confidence_interval(values[0]['processing_time']), wmci= mean_confidence_interval(values[0]['waiting_time'])))
        else:
...
#    output_file_path = os.path.join('output_files','results')
    ...
    sup.create_csv_file_header(task_duration, os.path.join('output_files','task_duration.csv'))
    ...
    sup.create_csv_file_header(process_duration, os.path.join('output_files','process_duration.csv'))
    # Resources

    sources =  sorted(list(set([x['source'] for x in process_stats])))
    roles=sorted(list(set([x['resource'] for x in process_stats])))
    role_time_use = sta.role_statistics(process_stats)
    role_use = calculate_role_durations(sources, roles, role_time_use)
    sup.create_csv_file_header(role_use, os.path.join('output_files','role_use.csv'))
